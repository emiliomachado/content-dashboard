...
const recentPosts = [
  { title: "NCHA Strategic Partners — EmmiTec.Health selected as approved partner", reactions: "18", comments: "6", reposts: "3", lang: "EN", brand: "EmmiTec.Health" },
  { title: "Debate: Desenvolvendo o Mercado da Longevidade — HIHUB.TECH", reactions: "15", comments: "—", reposts: "1", lang: "PT", brand: "EmmiTec.Health" },
  { title: "O Tribunal de Contas da União — auditoria CONITEC / SUS", reactions: "15", comments: "—", reposts: "1", lang: "PT", brand: "EmmiTec.Health" },
  { title: "Inovar tambem e educar — telemedicina e saude digital", reactions: "16", comments: "—", reposts: "1", lang: "PT", brand: "EmmiTec.Health" },
];
...
const contentIdeas = [
  ...
  { title: "PETJourney: como nasceu a ideia de digitalizar a saude animal", lang: "PT", theme: "PETJourney", status: "draft" },
];

const brandColors: Record<string, string> = {
  "EmmiTec.Health": "text-blue-600",
  "PETJourney": "text-emerald-600",
};
              ...
              <div key={i} className="grid grid-cols-5 py-3 border-b border-border last:border-0 items-center gap-2">
                <div className="col-span-2 space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-semibold ${langColors[post.lang]}`}>{post.lang}</span>
                  </div>
                </div>
                <div className="flex items-center justify-center gap-1 text-sm">
                  <ThumbsUp className="h-3 w-3 text-muted-foreground" />{post.reactions}
                </div>
                <div className="flex items-center justify-center gap-1 text-sm">
                  <MessageSquare className="h-3 w-3 text-muted-foreground" />{post.comments}
                </div>
                <div className="flex items-center justify-center gap-1 text-sm">
                  <Repeat2 className="h-3 w-3 text-muted-foreground" />{post.reposts}
                </div>
              </div>
            ))}
          </div>
            ...
            {contentIdeas.map((idea, i) => (
              <div key={i} className="grid grid-cols-4 py-3 border-b border-border last:border-0 items-center gap-2">
                ...
                <div className="flex items-center justify-end gap-2">
                  <span className={`text-xs font-semibold ${langColors[idea.lang]}`}>{idea.lang}</span>
